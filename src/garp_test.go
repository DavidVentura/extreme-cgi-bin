...
func TestBroadcastAddr(t *testing.T) {
	...
	want := []byte{192, 168, 1, 255}
	res := broadcastAddr(ip, slash24)
	if !bytes.Equal(res, want) {
		t.Fatalf("bad broadcast: %s", res)
	}
}
...
func TestGarpPayload(t *testing.T) {
	...
	want := []byte{
		...
		bIP[0], bIP[1], bIP[2], bIP[3], // receiver ip (broadcast)
	}
